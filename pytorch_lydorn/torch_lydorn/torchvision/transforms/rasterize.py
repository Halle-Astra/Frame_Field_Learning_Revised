...
def draw_polygons(polygons, shape, fill=True, edges=True, vertices=True, line_width=3, antialiasing=False):
    assert type(polygons) == list, "polygons should be a list"
    import os
    # print(f'FILE:{os.path.split(__file__)[-1]}, length of var polygons is ', len(polygons))
    if len(polygons)==0:
        import multiprocess
        print('multiprocess name:', multiprocess.current_process().name, 'This process have no polygons.')
    # print(f'FILE:{__file__}, var inspecting, shapely\'s Polygon is ', shapely.geometry.Polygon)
