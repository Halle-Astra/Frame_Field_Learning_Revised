...
def skeletons_to_tensorskeleton(skeletons_batch: List[Skeleton], device: str=None) -> TensorSkeleton:
    ...
    for batch_i, skeleton in enumerate(skeletons_batch):
        ...
        import os
        print(f"FILE:{os.path.split(__file__)[-1]}, var inspecting, batchsize:{batch_size}, batch_i:{batch_i}, skeleton paths indptr's type and shape:{type(skeleton.paths.indptr),skeleton.paths.indptr.shape}")
        # skeleton.paths.indptr.type is ndarray, shape is (?,)
        ...
        print(f"FILE:{os.path.split(__file__)[-1]}, var inspecting, n_points:{n_points}")
        batch_list.append(batch_i * np.ones(n_points, dtype=np.long))  # 这是一个会报错的程序，容易触发矩阵内存过大，https://blog.csdn.net/u011444756/article/details/90112666
    ...
    print(f"FILE:{os.path.split(__file__)[-1]}, INFO: skeleton_batch processed.")
    ...
    print(f"FILE:{os.path.split(__file__)[-1]}, INFO: pos to tensor ing...")
    pos = torch.tensor(pos, dtype=torch.float, device=device)
    print(f"FILE:{os.path.split(__file__)[-1]}, INFO: pos to tensor finished.")
    degrees = torch.tensor(degrees, dtype=torch.long, device=device)
    print(f"FILE:{os.path.split(__file__)[-1]}, INFO: degrees to tensor finished.")
    path_index = torch.tensor(path_index, dtype=torch.long, device=device)
    print(f"FILE:{os.path.split(__file__)[-1]}, INFO: path_index to tensor finished.")
    path_delim = torch.tensor(path_delim, dtype=torch.long, device=device)
    print(f"FILE:{os.path.split(__file__)[-1]}, INFO: path_delim to tensor finished.")
    batch = torch.tensor(batch, dtype=torch.long, device=device)
    print(f"FILE:{os.path.split(__file__)[-1]}, INFO: batch to tensor finished.")
    batch_delim = torch.tensor(batch_delim_list, dtype=torch.long, device=device)
    print(f"FILE:{os.path.split(__file__)[-1]}, INFO: batch_delim to tensor finished.")
    ...
    print(f"FILE:{os.path.split(__file__)[-1]}, INFO: tensorpoly created.")
