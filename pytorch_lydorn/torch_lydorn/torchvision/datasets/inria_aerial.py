...
class InriaAerial(torch.utils.data.Dataset):
    ...
    #     for i in range(len(tile_info_list)):
    #         print('-----------the {}th is processing------------'.format(i))
    #         self._process_one(tile_info_list[i])
    ...
    def load_raw_data(self, tile_info):
        ...
        if self.gt_source == "disk":
            ...
            # if len(raw_data['gt_polygons']) == 0:
            #     print(
            #         f"Warning from FILE:{os.path.split(__file__)[-1]}, process name:{multiprocess.current_process().name},"
            #         f"gt_filepath:{gt_filepath} makes gt_polygons empty.")

            # print(
            # f"FILE:{os.path.split(__file__)[-1]}, process name:{multiprocess.current_process().name}, var inspecting, gt_filepath:{gt_filepath}, length is {len(raw_data['gt_polygons'])}.")
    ...
    def _process_one(self, tile_info):
        ...
        # print(f"\n--- {process_id} ---\n")
        # print(f'FILE:{os.path.split(__file__)[-1]}, var inspecting, process name:{multiprocess.current_process().name}')
        ...
        stats = {}
        # print('-----------process_id:{}-----------initializing'.format(process_id))
        ...
        if self.patch_size is not None:
            ...
            for i, bbox in enumerate(tqdm(patch_boundingboxes, desc=f"Patching {tile_name}", leave=False, position=process_id)):
                ...
                # print(f'process_id:{process_id},raw_data\'s shape is ', raw_data['image'].shape)
                # print(f'FILE:{os.path.split(__file__)[-1]}, process_id:{process_id},sample\'s shape is ', sample['image'].shape)
                # print(f"FILE:{__file__}, pre_transform is ", self.pre_transform)
                # print(f'FILE:{__file__}, flag of mask_only is ',self.mask_only)
                # print(f'FILE:{os.path.split(__file__)[-1]}, var inspecting, sample\'s length:{len(sample)}')
                # print(f'FILE:{os.path.split(__file__)[-1]}, var inspecting, sample\'s gt_polygons:{sample["gt_polygons"]}\n'
                #       f'gt_type:{self.gt_type}\n'
                #       f'raw_data\'s polygons:{raw_data["gt_polygons"]}')
                if True:# if len(raw_data["gt_polygons"]) == 0:
                    print(f'FILE:{os.path.split(__file__)[-1]}, process name:{multiprocess.current_process().name}, var inspecting, \n'
                      f'length of raw_data\'s polygons:{len(raw_data["gt_polygons"])}\n'
                      f'city:{tile_info["city"]}, number:{tile_info["number"]}')
                try:
                    sample = self.pre_transform(sample)  # Needs "image" to infer shape even if mask_only is True
                    print(
                        f'FILE:{os.path.split(__file__)[-1]}, process name:{multiprocess.current_process().name}, this process encountered errors. var inspecting, \n'
                        f'length of raw_data\'s polygons:{len(raw_data["gt_polygons"])}\n'
                        f'city:{tile_info["city"]}, number:{tile_info["number"]}, sample have been pre_transformed.')
                except:
                    print(
                        f'Warning from FILE:{os.path.split(__file__)[-1]}, process name:{multiprocess.current_process().name}, this process encountered errors. var inspecting, \n'
                        f'length of raw_data\'s polygons:{len(raw_data["gt_polygons"])}\n'
                        f'city:{tile_info["city"]}, number:{tile_info["number"]}')
                if self.mask_only:
        ...
        print('process {} is finished.'.format(process_id))
        return stats
    ...
    def __getitem__(self, idx):
        if self.pre_process:
            ...
            print(f"FILE:{os.path.split(__file__)[-1]}, var inspecting, filepath:{filepath}")
