...
def get_inria_aerial_folds(config, root_dir, folds):
    ...
    # import os ;print('FILE {}:Augmentation_enable in config:{}'.format(
    #     os.path.abspath(__file__), config['data_aug_params']['enable']))
    # print('-'*20+'config'+'-'*20)
    # print(config)
    # print('-'*20+'config'+'-'*20)
    online_cpu_transform = data_transforms.get_online_cpu_transform(config,
                                                                    augmentations=config["data_aug_params"]["enable"])
    # print(f'FILE:{__file__}, online_cpu_transform is ', online_cpu_transform)
