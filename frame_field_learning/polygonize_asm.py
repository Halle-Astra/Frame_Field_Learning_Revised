...
from torch_lydorn.torchvision.transforms import Paths, Skeleton, TensorSkeleton, skeletons_to_tensorskeleton, \
...
from . import skan_jingdong
DEBUG = False  # value set by author is False
debug_halle = True
...
def get_junction_corner_index(tensorskeleton):
    ...
    junction_edge_index = torch.empty((2 * tensorskeleton.num_paths, 2), dtype=torch.long,
                                      device=tensorskeleton.path_index.device)
    ...
    slice_start = 0
    junction_corner_index = torch.empty((grouped_junction_edge_index.shape[0], 3), dtype=torch.long,
                                        device=tensorskeleton.path_index.device)
...
class AlignLoss:
    def __init__(self, tensorskeleton: TensorSkeleton, indicator: torch.Tensor, level: float, c0c2: torch.Tensor,
                 loss_params):
        """
    ...
    def __call__(self, pos: torch.Tensor, iter_num: int):
        ...
        align_loss = frame_field_utils.framefield_align_error(midpoints_c0, midpoints_c2, normed_tangents,
                                                              complex_dim=1)
        ...
        with torch.no_grad():
            ...
            is_corner_index = torch.nonzero(is_corner)[:,
                              0] + 1  # Shift due to first vertex not being represented in is_corner
            ...
            sub_path_delim_is_corner = self.tensorskeleton.path_delim.shape[
                                           0] <= sub_path_sort_indices  # If condition is true, then the delimiter is from is_corner_index
        ...
        vertex_angles = torch.acos(dot) * torch.sign(
            det)  # TODO: remove acos for speed? Switch everything to signed dot product?
        # Save angles of detected corners:
        corner_angles = vertex_angles[
            is_corner_index - 1]  # -1 because of the shift of vertex_angles relative to path_pos
        ...
        vertex_angles[self.tensorskeleton.path_delim[
                      1:-1] - 2] = 0  # Zero out invalid angles between paths (caused by the junction points being in all paths of the junction)
        ...
        sub_path_mean_vertex_angles[
            sub_path_small_dissimilarity_mask] = 0  # Optimize sub-path with a small dissimilarity to have straight edges
        ...
        curvature_loss[self.tensorskeleton.path_delim[
                       1:-1] - 2] = 0  # Zero out loss for end vertex of inner paths (caused by the junction points being in all paths of the junction)
...
class TensorSkeletonOptimizer:
    ...
    def optimize(self) -> TensorSkeleton:
        if DEBUG:
            optim_iter = tqdm(range(self.config["loss_params"]["coefs"]["step_thresholds"][-1]),
                              desc="Gradient descent", leave=True)
...
def get_skeleton(np_edge_mask, config):
    ...
    import os
    # print(f"FILE:{os.path.split(__file__)[-1]}, var inspecting, shape of np_edge_mask:{np_edge_mask.shape}, mean of np_edge_mask:{np_edge_mask.mean()}")
    pad_width = 2
    ...
    if 0 < skeleton_image.sum():  # former threshold set by author is 0
        ...
        try:
            # import multiprocess
            # print(f"FILE:{os.path.split(__file__)[-1]}, var inspecting, skeleton_image's shape:{skeleton_image.shape}")
            skeleton_check_object = skan_jingdong.Skeleton_test(skeleton_image, keep_images = False)
            # print(f"FILE:{os.path.split(__file__)[-1]}, INFO: skan testing no problems.")
            if skeleton_check_object.paths.error:
                skeleton = Skeleton()
                print(f"FILE:{os.path.split(__file__)[-1]}, "
                      f"INFO: skan Skeleton will raise error, so an empty Skeleton is created.")
                # 关于share memory不足的提示：https://blog.csdn.net/ustczhng2012/article/details/115716254
            else:
                ...
                if skeleton.coordinates.shape[0] != skeleton.degrees.shape[0]:
                    print(
                        f"FILE:{os.path.split(__file__)[-1]}, INFO: ValueError will be raised as degree's shape and coordinates' shape is different.")
                    raise ValueError(
                        f"skeleton.coordinates.shape[0] = {skeleton.coordinates.shape[0]} while skeleton.degrees.shape[0] = {skeleton.degrees.shape[0]}. They should be of same size.")

            # # print(f"FILE:{os.path.split(__file__)[-1]}, INFO: Skeleton object will be created. process's name is {multiprocess.current_process().name}")
            # try:
            #     skeleton = skan.Skeleton(skeleton_image, keep_images=False)
            #     print(f"FILE:{os.path.split(__file__)[-1]}, var inspecting, number of skeleton's true values:{skeleton_image.sum()}")
            # except:
            #     skeleton = Skeleton()
            #     print(f"FILE:{os.path.split(__file__)[-1]}, INFO: Skeleton created failed.\n"  # 有出现
            #           f"\tvar inspectng, skeleton image:{skeleton_image}, its number of true values:{skeleton_image.sum()}")
            # # print(f"FILE:{os.path.split(__file__)[-1]}, INFO: Skeleton object finished creating. process's name is {multiprocess.current_process().name}")
            ...
            # if skeleton.coordinates and skeleton.paths:
            #     skeleton.coordinates = skeleton.coordinates[:skeleton.paths.indices.max() + 1]
            # # try:
            # #     skeleton.coordinates = skeleton.coordinates[:skeleton.paths.indices.max() + 1]
            # # except Exception as e:
            # #     # print(e)
            #     print(f"FILE:{os.path.split(__file__)[-1]}, INFO: skeleton's coordinates cannot be got. \n"
            #           f"\tvar inspecting, skeleton's coordinates:{skeleton.coordinates}, paths' indices:{skeleton.paths.indices}\n")
            #           # f"\tvar inspecting, skeleton is empty?{skeleton == Skeleton()}\n")
            #           # f"\tvar inspecting, shape of coordinates:{skeleton.coordinates.shape[0]}, shape of degrees:{skeleton.degrees.shape}.")
            ...
            pass
            # np.save('skeleton', skeleton_image)
            skeleton = Skeleton()
            print(f"FILE:{os.path.split(__file__)[-1]}, INFO: Error encountered and will be passed.\t{e}")
    else:
        print(f"FILE:{os.path.split(__file__)[-1]}, INFO: an empty skeleton will be created.")
        skeleton = Skeleton()
    # toc = time.time()
    ...
    # print(f"FILE:{os.path.split(__file__)[-1]}, INFO: skeleton will be returned. skeleton image's sum:{skeleton_image.sum()}")
    import multiprocess
    # print(f"FILE:{os.path.split(__file__)[-1]}, var inspecting, process's name:{multiprocess.current_process().name}\n"
    #       f"\tcoordinates' shape:{skeleton.coordinates.shape}, degrees' shape:{skeleton.degrees.shape}")
    if 'skeleton_check_object' in dir() and skeleton_check_object.paths.error:
        print(f"FILE:{os.path.split(__file__)[-1]}, INFO: check error happend, but will return normally with an empty object.")
    return skeleton
...
def get_marching_squares_skeleton(np_int_prob, config):
    ...
    """
    # tic = time.time()
    contours = skimage.measure.find_contours(np_int_prob, config["data_level"], fully_connected='low',
...
def compute_skeletons(seg_batch, config, spatial_gradient, pool=None) -> List[Skeleton]:
    ...
    if config["init_method"] == "marching_squares":
        ...
        corrected_edge_prob_batch = corrected_edge_prob_batch[:, None, :,
                                    :].float()  # Convet to float for spatial grads
        corrected_edge_prob_batch = 2 * spatial_gradient(corrected_edge_prob_batch)[:, 0, :,
                                        :]  # (b, 2, h, w), Normalize (kornia normalizes to -0.5, 0.5 for input in [0, 1])
        ...
        if pool is not None:
            ...
            import multiprocess
            print(
                f"FILE:{os.path.split(__file__)[-1]}, INFO: pool is not None. var inspecting, process name:{multiprocess.current_process().name}")
            print(
                f"FILE:{os.path.split(__file__)[-1]}, var inspecting, the shape of edge_mask_batch:{np_corrected_edge_mask_batch.shape}")
            skeletons_batch = pool.map(get_skeleton_partial, np_corrected_edge_mask_batch)
        else:
            import os
            print(f"FILE:{os.path.split(__file__)[-1]}, INFO: get_skeleton_partial will be executed.")
            if debug_halle:
                skeletons_batch = []
                for np_corrected_edge_mask_batch_index, np_corrected_edge_mask_batch_i in enumerate(
                        np_corrected_edge_mask_batch):
                    res = get_skeleton(np_corrected_edge_mask_batch_i, config=config)
                    # print(f'FILE:{os.path.split(__file__)[-1]}, INFO: {np_corrected_edge_mask_batch_index}th edge mask processed.')
                    skeletons_batch.append(res)
                    # print(
                    #     f"FILE:{os.path.split(__file__)[-1]}, var inspecting, memory size of res:{sys.getsizeof(res)/1048576}MB.")

                del get_skeleton_partial

                #查看变量大小的方法https://xercis.blog.csdn.net/article/details/104757467
                print(f"FILE:{os.path.split(__file__)[-1]}, var inspecting, memory size of skeletons:{sys.getsizeof(skeletons_batch)/1048576}MB.")
            else:
                skeletons_batch = list(map(get_skeleton_partial, np_corrected_edge_mask_batch))
            print(f"FILE:{os.path.split(__file__)[-1]}, INFO: get skeleton finished.")
        # toc = time.time()
        # debug_print(f"get_skeleton_polylines: {toc - tic}s")
    else:
        raise NotImplementedError(
            f"init_method '{config['init_method']}' not recognized. Valid init methods are 'skeleton' and 'marching_squares'")
...
class PolygonizerASM:
    ...
    def __call__(self, seg_batch, crossfield_batch, pre_computed=None):
        ...
        import os
        print(f"FILE:{os.path.split(__file__)[-1]}, var inspecting, skeleton batch:length:{len(skeletons_batch)}, type:{type(skeletons_batch)}")  # toc = time.time()
        ...
        if not debug_halle:
            ...
            print(f"FILE:{os.path.split(__file__)[-1]}, INFO: Will not do skeleton_to_tensorskeleton.")
            return None, None
        ...
        if DEBUG:
            ...
            optim_pbar = tqdm(desc="Gradient descent", leave=True,
                              total=self.config["loss_params"]["coefs"]["step_thresholds"][-1])
            ...
            ani = animation.FuncAnimation(
                fig, animate, init_func=init, interval=0, blit=True,
                frames=self.config["loss_params"]["coefs"]["step_thresholds"][-1], repeat=False)
...
def main():
    ...
    if args.raw_pred is not None:
        ...
        for seg_filepath, angles_map_filepath in zip(seg_filepaths, angles_map_filepaths):
            ...
            seg = input_seg[:, :,
                  [1, 2]]  # LuxCarta channels are (background, interior, wall), re-arrange to be (interior, wall)
        ...
        plot_utils.save_poly_viz(seg[:, :, 0], polygons, filepath, linewidths=0.5, draw_vertices=True,
