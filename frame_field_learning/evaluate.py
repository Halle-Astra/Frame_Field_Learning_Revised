...
if debug_halle:
    from .evaluator_jingdong import Evaluator
else:
...
def evaluate(gpu: int, config: dict, shared_dict, barrier, eval_ds, backbone):
    ...
    eval_ds = torch.utils.data.DataLoader(eval_ds, batch_size=config["optim_params"]["eval_batch_size"], pin_memory=False)#, sampler=eval_sampler, num_workers=config["num_workers"])
    ...
    # model = model.to('cuda')# replaced by halle
