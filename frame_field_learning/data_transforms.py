...
def get_online_cpu_transform(config, augmentations=False):
    """在Inria数据的config（config.inria_dataset_osm_mask_only.unet16.json）：
    augmentations:True
    device:cuda
    affine:True
    patch_size:True
    ...
    import os
    print(f"FILE:{os.path.split(__file__)[-1]}, augmentations:{augmentations}\n"
          f"device:{config['data_aug_params']['device']}\n"
          f"affine:{config['data_aug_params']['affine']}\n"
          f"patch_size:{'input_patch_size' in config['dataset_params']}\n"
          f"vflip:{config['data_aug_params']['vflip']}\n"
          f"colorjitter:{config['data_aug_params']['color_jitter']}")
    ...
    import os
    # print(f"FILE:{os.path.split(__file__)[-1]}, transform_list:{online_transform_list}")
