...
def get_args():
    ...
    argparser.add_argument(
        '-c', '--config',
        type=str,
        default="configs/config.mapping_dataset.unet16.json",
...
def launch_train(args):
    ...
    # --- Load params in config set as relative path to another JSON file
    print(f"FILE:{os.path.split(__file__)[-1]}, condition checking, defaults_filepath in config:{'defaults_filepath'in config}.")
    config = run_utils.load_defaults_in_config(config, filepath_key="defaults_filepath")
...
def launch_eval(args):
    ...
    print(f"FILE:{os.path.split(__file__)[-1]}, var inspecting, num workers:{config['num_workers']}, num gpus:{config['gpus']}.")
    ...
    # barrier = manager.Barrier(args.gpus)#https://blog.csdn.net/weixin_33885676/article/details/90326954
    from Barrier_jingdong import Barrier
    barrier = Barrier()
    if debug_halle:
        args.gpus = 0
    ...
    # torch.multiprocessing.set_sharing_strategy('file_system')# possible problem https://blog.csdn.net/xixiaoyaoww/article/details/112645996
    print(f"FILE:{os.path.split(__file__)[-1]}, INFO: Barrier created successfully.")
    ...
    # web about cancel_join_thread is placed at `async_utils.py`.
